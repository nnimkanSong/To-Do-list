...
import { TermsDialog } from "@/components/ui/TermsDialog"; // ✅ ให้ path ตรงกับไฟล์คุณ
...
type PwCheck = { ok: boolean; label: string };

function passwordChecks(pw: string): PwCheck[] {
  return [
    { ok: pw.length >= 8, label: "อย่างน้อย 8 ตัวอักษร" },
    { ok: /[A-Z]/.test(pw), label: "มีตัวอักษรพิมพ์ใหญ่ (A–Z)" },
    { ok: /[a-z]/.test(pw), label: "มีตัวอักษรพิมพ์เล็ก (a–z)" },
    { ok: /\d/.test(pw), label: "มีตัวเลข (0–9)" },
    { ok: /[^A-Za-z0-9]/.test(pw), label: "มีอักขระพิเศษ (!@#$...)" },
  ];
}

function passwordScore(pw: string) {
  const items = passwordChecks(pw);
...
export default function RegisterForm() {
  ...
  const { passed, total, items } = useMemo(() => passwordScore(password), [password]);
  const strongEnough = passed >= 4; // ✅ ผ่านอย่างน้อย 4 ข้อถือว่าโอเค
  const passwordsMatch = confirm.length === 0 || password === confirm;

  // แนะนำโดเมนที่น่าจะพิมพ์ผิด
  useEffect(() => {
    const s = suggestEmailDomain(email.trim());
    setEmailSuggestion(s && s !== email.trim() ? s : null);
  }, [email]);

  // นับถอยหลัง cooldown ปุ่ม Send
  useEffect(() => {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!accepted) {
      alert("กรุณายอมรับเงื่อนไขการใช้งาน (Terms of Service)");
      return;
    }
    if (!strongEnough) {
      alert("รหัสผ่านยังไม่แข็งแรงพอ");
      return;
    }
    if (!passwordsMatch) {
  ...
  return (
    <form onSubmit={onSubmit} className="w-full max-w-sm rounded-lg text-card-foreground space-y-4">
      {/* Email */}
      ...
      {!accepted && (
        <p className="text-xs text-destructive/90">You must accept the Terms of Service to continue.</p>
      )}
      ...
      <Button
        type="submit"
        className="w-full"
        disabled={!accepted || !passwordsMatch || !strongEnough || submitting}
      >
